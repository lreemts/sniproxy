...
static struct test_packet good[] = {
    { (char *)good_data_1, sizeof(good_data_1), "localhost" },
    { (char *)good_data_2, sizeof(good_data_2), "localhost" },
    { (char *)good_data_3, sizeof(good_data_3), "localhost" },
    { (char *)alpn_good_data_1, sizeof(alpn_good_data_1), "http/2.0" }
};
...
int main() {
    ...
    struct TLSProtocol *tls_data = new_tls_data();
    assert(tls_data != NULL);
    tls_data = tls_data_append_alpn_protocol(tls_data, "http/1.1", 8);
    assert(tls_data != NULL);
    tls_data = tls_data_append_alpn_protocol(tls_data, "http/2.0", 8);
    assert(tls_data != NULL);
    tls_data = tls_data_append_alpn_protocol(tls_data, "spdy/3", 6);
    assert(tls_data != NULL);
    tls_data = tls_data_prefer_alpn(tls_data, 1);

    for (i = 0; i < sizeof(good) / sizeof(struct test_packet); i++) {
        hostname = NULL;

        result = tls_protocol->parse_packet(tls_data, good[i].packet, good[i].packet_len, &hostname);

        assert(result == strlen(good[i].expected_name));

        assert(NULL != hostname);

        if (strcmp(good[i].expected_name, hostname) != 0) {
            fprintf(stderr, "test %d: received \"%s\", expected \"%s\"\n", i, hostname, good[i].expected_name);
            assert(0);
        }
    ...
    result = tls_protocol->parse_packet(NULL, good[0].packet, good[0].packet_len, NULL);
    assert(result == -3);

    for (i = 0; i < sizeof(bad) / sizeof(struct test_packet); i++) {
        hostname = NULL;

        result = tls_protocol->parse_packet(tls_data, bad[i].packet, bad[i].packet_len, &hostname);
